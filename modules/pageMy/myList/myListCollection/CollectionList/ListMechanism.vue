	...
	<view class="ListMechanism">
		...
		<view class="ListMechanismfist" v-for="(items,index) in List.rows" :key="index" @tap="gotojiguo(items.id,items.title)" @longpress="deleat(items.id)">
			...
			<view>
				...
				<view class="ListMechanismtwo">最近投资：<span v-for="(ite,index) in items.inves" :key="index">{{ite}}、</span></view>
			</view>
		</view>
		...
		methods: {
			...
			getConcang(e) {
				if (uni.getStorageSync('landRegist')) {
					...
					uni.request({
						url: this.api2 + '/follow/capiList?userId='+landRegistLG.user.id+'&page=' + this.searchCondition.page, //接口地址。
						// data: this.endParams(params),
						method: 'GET',
						header: {
							Authorization: "Bearer " + landRegistLG.token //将token放到请求头中
			...
			getMoreList(e){
				...
				if (uni.getStorageSync('landRegist')) {
					...
					uni.request({
						url: this.api2 + '/follow/capiList?userId='+landRegistLG.user.id+'&page=' + e.search.searchCondition.page, //接口地址。
						data: this.endParams(params),
						method: 'POST',
						header: {
							Authorization:"Bearer "+landRegistLG.token//将token放到请求头中
