		...
		methods: {
			...
			getConcang(e) {
				if (uni.getStorageSync('landRegist')) {
					...
					uni.request({
						url: this.api2 + '/follow/newsList?userId='+landRegistLG.user.id+'&page=' + this.searchCondition.page, //接口地址。
						// data: this.endParams(params),
						method: 'GET',
						header: {
							Authorization: "Bearer " + landRegistLG.token //将token放到请求头中
			...
			getMoreList(e){
				...
				if (uni.getStorageSync('landRegist')) {
					...
					uni.request({
						url: this.api2 + '/follow/newsList?userId='+landRegistLG.user.id+'&page=' + e.search.searchCondition.page, //接口地址。
						data: this.endParams(params),
						method: 'POST',
						header: {
							Authorization:"Bearer "+landRegistLG.token//将token放到请求头中
